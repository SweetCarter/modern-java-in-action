...
import java8demo.d_并发增强.组合式异步编程.最佳价格查询器.v2.ExchangeService;
import java8demo.d_并发增强.组合式异步编程.最佳价格查询器.v2.ExchangeService.Money;
...
public class BestPriceFinder {
	...
	{
		final int coreThreads = Math.min(shops.size(), 100);
		executor = Executors.newFixedThreadPool(coreThreads, new ThreadFactory() {
			@Override
	...
	public List<String> findPricesFuture(String product) {
		List<CompletableFuture<String>> priceFutures =
				shops.stream()
						...
						.collect(toList());
//使用同一个stream连续使用map会导致CompletableFuture顺序创建并执行
